...
# 开发环境兜底（没有环境变量时用当前目录）
if not TRIM_APPDEST:
    TRIM_APPDEST = os.path.dirname(os.path.abspath(__file__))
if not TRIM_PKGVAR:
    TRIM_PKGVAR = os.path.join(TRIM_APPDEST, 'data')
...
# vendor 路径
# ============================================
VENDOR_PATH = os.path.join(TRIM_APPDEST, 'vendor')
...
# ============ Flask 初始化 ============
TEMPLATE_FOLDER = os.path.join(TRIM_APPDEST, 'templates')
...
def get_upload_folder():
    ...
    default_path = os.path.join(TRIM_APPDEST, 'uploads')
...
with app.app_context():
    ...
    defaults = ['文档资料', '软件工具', '视频教程', '设计素材', '其他']
...
def static_file(filename):
    ...
    if ext in allowed_ext:
        return send_from_directory(TRIM_APPDEST, filename)
...
def admin_add_resource():
    ...
    safe_display_name = re.sub(r'[<>:"/\\|?*]', '_', original_filename).strip()
    if not safe_display_name:
        safe_display_name = 'file'
    ...
    if not safe_storage_name:
        ...
        if '.' in original_filename:
            ...
            safe_storage_name = f'{timestamp}.{ext}'
        else:
            safe_storage_name = f'{timestamp}'
    else:
        safe_storage_name = f'{timestamp}_{safe_storage_name}'

    file_path = os.path.join(app.config['UPLOAD_FOLDER'], safe_storage_name)
    file.save(file_path)
